...
| Needs pods to reach each other across regions | `globalNetworking: true` → `globalNetworking.{ip, internalDns}` (`<podId>.runpod.internal`) | Private cross-datacenter networking with a DNS name, returned on the pod. |
...
| Has ad-hoc rate-limit backoff | `ratelimit` / `ratelimit-policy` response headers on every authenticated response | e.g. `"minute";r=176;t=1, "hour";r=7193;t=2701` — throttle proactively instead of reacting to `429`s. |
