...
SIGNALS: list[tuple[str, str, str, str]] = [
    ...
    ("v1-field", "pod", r"\bcountryCodes\b", "No create-time equivalent, but there IS a migration: filter /v2/catalog/gpus?include=AVAILABILITY&countryCodes=.. to get the matching data centers, pass their IDs as dataCenterIds on create, then assert the created pod's dataCenterId is in that set. Working code: breaking-changes.md -> Replacing countryCodes. Ask the user whether this was a preference or a compliance requirement before choosing how hard to enforce it."),
