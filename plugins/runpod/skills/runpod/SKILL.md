...
description: >-
  ...
  companion-clis, runpod-usage, runpod-migrate, runpod-comfyui-models) and indexes
  two dozen live-verified end-to-end examples (golden paths) — use it when the lane
  is unclear, and for any multi-step or provisioning task even when it is not.
metadata:
  author: runpod
...
2. **Repair an imported ComfyUI workflow or output PNG whose referenced models
