...
- **Leave `--cloud-type` at its `SECURE` default.** Community Cloud is cheaper but less stable (hosts reclaimed mid-run, public IPs change on migrate/restart, port 22 needs `--public-ip`). Only pass `--cloud-type COMMUNITY` when the user explicitly asks for it, and say what they are trading away.
